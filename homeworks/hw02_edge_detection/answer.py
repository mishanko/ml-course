...
def compute_hog(image, pixels_per_cell=(cell_size, cell_size), bins=9):
    ...
    for i in range(n_cells_y):
        for j in range(n_cells_x):
            cell_magnitudes = magnitude[i * cell_height:(i + 1) * cell_height, j * cell_width:(j + 1) * cell_width]
            cell_directions = direction[i * cell_height:(i + 1) * cell_height, j * cell_width:(j + 1) * cell_width]
            histogram = np.histogram(cell_directions, bins=bins, range=(-180, 180), weights=cell_magnitudes)[0]
            histogram = histogram / (histogram.sum() + 1e-15)
