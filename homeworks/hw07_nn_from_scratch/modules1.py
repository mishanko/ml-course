...
class Linear(Module):
    ...
    def updateOutput(self, input):
        # Your code goes here. ################################################
        # self.output = ...
        self.output = np.add(input @ self.W.T, self.b.T)
        return self.output
    ...
    def accGradParameters(self, input, gradOutput):
        ...
        self.gradW = np.sum(
            np.multiply(input[:, None, :], gradOutput[:, :, None]), axis=0
        )
...
class SoftMax(Module):
    ...
    def updateOutput(self, input):
        ...
        self.output = np.divide(
            np.exp(self.output), np.sum(np.exp(self.output), axis=1, keepdims=True)
        )
        return self.output

    def updateGradInput(self, input, gradOutput):
        # Your code goes here. ################################################
        batch_size, n_features = gradOutput.shape
        l_softmax = self.output
        tensor1 = np.einsum(
            "ij,ik->ijk", l_softmax, l_softmax
        )  # (batch_size, n_features, n_features)
        tensor2 = np.einsum(
            "ij,jk->ijk", l_softmax, np.eye(n_features, n_features)
        )  # (batch_size, n_features, n_features)
        dSoftmax = np.subtract(tensor2, tensor1)
        self.gradInput = np.einsum(
            "ijk,ik->ij", dSoftmax, gradOutput
...
class LogSoftMax(Module):
    ...
    def updateGradInput(self, input, gradOutput):
        # Your code goes here. ################################################
        self.gradInput = np.subtract(
            gradOutput,
            np.multiply(np.exp(self.output), np.sum(gradOutput, axis=1, keepdims=True)),
        )
        return self.gradInput
...
class BatchNormalization(Module):
    ...
    def updateOutput(self, input):
        ...
        if self.training:
            ...
            #     self.output = np.divide(np.subtract(input, batch_mean), np.sqrt(batch_variance + self.EPS))

            #     self.moving_mean = np.add(np.multiply(self.moving_mean, self.alpha), np.multiply(batch_mean, (1 - self.alpha)))
            #     self.moving_variance = np.add(np.multiply(self.moving_variance, self.alpha), np.multiply(batch_variance, (1 - self.alpha)))
            # else:
            #     self.output = np.divide(np.subtract(input, self.moving_mean), np.sqrt(self.moving_variance + self.EPS))
            batch_mean = input.mean(axis=0)
            batch_variance = input.var(axis=0)
    ...
    def updateGradInput(self, input, gradOutput):
        ...
        # variable0 = np.subtract(input, batch_mean)
        # variable1 = np.sum(np.multiply(gradOutput, variable0), axis=0)
        # variable2 = np.sum(gradOutput, axis=0)
        # variable3 = np.sqrt(batch_variance + self.EPS)

        # self.gradInput = np.divide(gradOutput, variable3)
        # self.gradInput -= np.divide(np.divide(np.divide(np.multiply(variable1, variable0), m), variable3), (batch_variance + self.EPS))
        # self.gradInput -= np.divide(np.divide(variable2, m), variable3)
        # self.gradInput += np.divide(np.divide(np.multiply(variable1, np.sum(variable0, 0)), np.pow(m, 2)), np.pow(variable3, 3/2))
        batch_mean = np.mean(input, axis=0) if self.training else self.moving_mean
        batch_variance = (
            np.var(input, axis=0) if self.training else self.moving_variance
        )
        m = input.shape[0]
        ...
        self.gradInput -= (
            variable1 * variable0 / m / variable3 / (batch_variance + self.EPS)
        )
...
class LeakyReLU(Module):
    ...
    def updateOutput(self, input):
        # Your code goes here. ################################################
        self.output = (
            np.maximum(input, np.multiply(self.slope, input))
            if self.slope <= 1
            else np.minimum(input, np.multiply(self.slope, input))
        )
        return self.output
    ...
    def updateGradInput(self, input, gradOutput):
        ...
        self.gradInput = np.where(
            input > 0, gradOutput, np.multiply(gradOutput, self.slope)
        )
        return self.gradInput
...
class ELU(Module):
    ...
    def updateOutput(self, input):
        ...
        self.output = np.where(
            input > 0, input, np.multiply(self.alpha, (np.exp(input) - 1))
        )
        return self.output
    ...
    def updateGradInput(self, input, gradOutput):
        ...
        self.gradInput = np.where(
            input > 0,
            gradOutput,
            np.multiply(np.multiply(gradOutput, self.alpha), np.exp(input)),
        )
        return self.gradInput
...
class SoftPlus(Module):
    ...
    def updateGradInput(self, input, gradOutput):
        # Your code goes here. ################################################
        self.gradInput = np.multiply((1 / (1 + np.exp(-input))), gradOutput)
        return self.gradInput
...
class ClassNLLCriterionUnstable(Criterion):
    ...
    def updateOutput(self, input, target):
        ...
        # Your code goes here. ################################################
        self.output = (
            -1 / target.shape[0] * np.sum(np.multiply(target, np.log(input_clamp)))
        )
        return self.output
    ...
    def updateGradInput(self, input, target):
        ...
        # Your code goes here. ################################################
        self.gradInput = -1 / input.shape[0] * np.divide(target, input_clamp)
        return self.gradInput
...
class ClassNLLCriterion(Criterion):
    ...
    def updateOutput(self, input, target):
        # Your code goes here. ################################################
        self.output = -1 / target.shape[0] * np.sum(np.multiply(target, input))
        return self.output
...
class Conv2d(Module):
    ...
    def updateOutput(self, input):
        ...
        self.output = (
            scipy.signal.correlate(zero_pad_img, self.W, mode="valid") + self.b
        )
        return self.output
    ...
    def updateGradInput(self, input, gradOutput):
        ...
        # self.gradInput = ...
        zero_pad_gradOutput = np.pad(
            gradOutput, ((pad_size, pad_size), (pad_size, pad_size), (0, 0)), "constant"
        )
        self.gradInput = scipy.signal.correlate(
            zero_pad_gradOutput, self.output, mode="valid"
        )
        return self.gradInput
