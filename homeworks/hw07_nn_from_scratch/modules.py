...
class BatchNormalization(Module):
    ...
    def updateOutput(self, input):
        ...
        if self.training:
            ...
            self.output = np.divide(
                np.subtract(input, batch_mean), np.sqrt(batch_variance + self.EPS)
            )

            self.moving_mean = np.add(
                np.multiply(self.moving_mean, self.alpha),
                np.multiply(batch_mean, (1 - self.alpha)),
            )
            self.moving_variance = np.add(
                np.multiply(self.moving_variance, self.alpha),
                np.multiply(batch_variance, (1 - self.alpha)),
            )
        else:
            self.output = np.divide(
                np.subtract(input, self.moving_mean),
    ...
    def updateGradInput(self, input, gradOutput):
        ...
        self.gradInput -= np.divide(
            np.divide(np.divide(np.multiply(variable1, variable0), m), variable3),
            (batch_variance + self.EPS),
        )
        self.gradInput -= np.divide(np.divide(variable2, m), variable3)
        self.gradInput += np.divide(
            np.divide(np.multiply(variable1, np.sum(variable0, 0)), np.pow(m, 2)),
            np.pow(variable3, 3 / 2),
        )
        return self.gradInput
